...
```{r data}
samples <- read_excel(paste0(STR_DATA,"Q3.xlsx"), sheet = 1) %>%
  mutate(
...
counts <- read_excel(paste0(STR_DATA,"Q3.xlsx"), sheet = 3) %>%
  rename(
    !!! setNames(samples$SegmentDisplayName, samples$segmentname),
    gene = TargetName
  )

targets <- read_excel(paste0(STR_DATA,"Q3.xlsx"), sheet = 2)
...
# PCA analysis across all samples and counts, including information about tumor vs stroma
...
selected_samples <- counts %>%
  ...
  left_join(select(samples, segment = segmentname, patient = Patient, celltype, region = Region), by = "segment") %>%
...
pca_samples %>%
  ...
  ggplot(aes(.fittedPC1, .fittedPC2, color = patient)) + 
  geom_point(aes(shape = celltype), size = 2) +
  labs(title = "PCA", color = "Patient", shape = "Tumor/Stroma") + 
  xlab("PC1") + ylab("PC2")
```